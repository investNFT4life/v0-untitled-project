  ...
  return (
    <Card className="bg-transparent border-0 overflow-hidden relative shadow-xl">
      ...
      <CardContent className="p-6 relative bg-[#121212]/80 backdrop-blur-sm">
        ...
        <h3 className="text-2xl font-bold mb-1 text-[#00BFFF]">{nft.name}</h3>
        {nft.subtitle && <p className="text-white text-sm mb-3">{nft.subtitle}</p>}

        <div className="mb-4">
          <div>
            <p className="text-white text-sm">Price</p>
            <p className="text-xl font-bold text-[#00BFFF]">{nft.price}</p>
            <p className="text-sm text-white">{nft.euroPrice}</p>
            {nft.availability && <p className="text-sm text-[#40E0FF] mt-1 font-medium">{nft.availability}</p>}
          </div>
        </div>
        ...
        {nft.benefits && (
          <div className="my-4">
            <p className="font-semibold mb-2 text-transparent bg-clip-text bg-gradient-to-r from-[#00BFFF] to-[#40E0FF]">
              {nft.id.charAt(0).toUpperCase() + nft.id.slice(1)} Benefits
            </p>
            <ul className="space-y-1">
              {nft.benefits.map((benefit, index) => (
                <li key={index} className="flex items-start">
                  <Check className="h-4 w-4 mr-2 mt-1 text-[#00BFFF]" />
        ...
        <div className="flex flex-col sm:flex-row gap-2 mt-4">
          <Dialog>
            ...
            <DialogContent className="bg-[#121212] text-white border border-[#00BFFF]/20 backdrop-blur-lg">
              <DialogHeader>
                <DialogTitle className="text-2xl text-[#00BFFF]">{nft.name}</DialogTitle>
                ...
                <form className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-2 bg-black border border-[#00BFFF]/20 rounded-md focus:border-[#00BFFF]/50 focus:outline-none focus:ring-1 focus:ring-[#00BFFF]/30"
                    />
                  </div>
                  ...
                  <Button className="w-full bg-[#00BFFF] hover:bg-[#40E0FF] text-black font-medium mt-4">
